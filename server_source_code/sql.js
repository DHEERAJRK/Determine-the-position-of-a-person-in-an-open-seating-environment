 ...
 /* FUTURE WORK */
  ...
  //method to insert an newly joined employee details by admin
    ...
    app.post("/search/:term", function(req, res) {
        var Ename = req.params.term;

        var strQuery = "SELECT r.Floor,r.Pillar FROM Employee e,Dynamic_Routing d,RouterIp r WHERE (e.First_name LIKE ? AND e.EMac_Addr=d.EMac_Addr AND d.RMac_Addr=r.RMac_Addr)";
         connection.query( strQuery,'%'+Ename+'%', function(err, result){
          ...
          }else
		  {
			  ...
			  var jsonr=json.replace(/\]/g, '');
			  var jsonr1=jsonr.replace(/\{/g, '');
			  var jsonr2=jsonr1.replace(/\}/g, '');
            console.log(jsonr2);
            res.end(jsonr2);
          }
         });
...
function updateData(Emac,Rmac,connection,res){
	var strQuery = connection.query('UPDATE Dynamic_Routing SET RMac_Addr= ? WHERE EMac_Addr = ?', [Rmac,Emac], function (err, result) {
          if (err) {
          return;
        }
        res.end("successfull");
